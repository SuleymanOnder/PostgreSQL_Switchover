...
# Usage: sh SwitchOverPostgreSQL.sh SourceDC=DC1 TargetDC=DC2
# Author: Suleyman
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        *)
        ...
        echo "Usage: $0 SourceDC=[DC2|DC1] TargetDC=[DC2|DC1]"
        exit 1
        ;;
    esac
done
...
if [[ -z "$SOURCE_DC" ]] || [[ -z "$TARGET_DC" ]]; then
    echo "Error: Both SourceDC and TargetDC must be specified"
    echo "Usage: $0 SourceDC=[DC2|DC1] TargetDC=[DC2|DC1]"
    exit 1
fi
...
if ! check_permissions; then
    log_message "Permission checks failed. Exiting."
    exit 1
fi
...
echo "$UPDATED_STATUS" | grep -A1 "${SOURCE_PREFIX}" | grep -v "Leader" | grep "^|" | while read -r line; do
    node=$(echo "$line" | awk -F'|' '{print $2}' | tr -d ' ')
    if [ ! -z "$node" ] && [ "$node" != "|" ]; then
...
if [ "$SOURCE_NOSYNC_FALSE_COUNT" -gt 0 ]; then
    ...
    COUNT=0
    echo "$NEW_STATUS" | grep -A1 "${TARGET_PREFIX}" | grep -v "Leader" | grep "^|" | while read -r line; do
        node=$(echo "$line" | awk -F'|' '{print $2}' | tr -d ' ')
        if [ ! -z "$node" ] && [ "$node" != "|" ]; then
...
echo "$NEW_STATUS" | grep -A1 "${SOURCE_PREFIX}" | grep "^|" | while read -r line; do
    node=$(echo "$line" | awk -F'|' '{print $2}' | tr -d ' ')
    if [ ! -z "$node" ] && [ "$node" != "|" ]; then
        update_tags "$node" "true" "true"
        wait_for_cluster_stability
...
log_message "Performed By: $(whoami)"
